...
#                 else:
#                     logging.warning(
#                         f"Failed to fetch {url} with status code {response.status_code}. Retrying... (Attempt {attempt}/{retries})"
#                     )
#                     time.sleep(random.uniform(1, 3))
#             except requests.exceptions.RequestException as e:
#                 logging.error(
#                     f"An error occurred while fetching {url}: {e}. Retrying... (Attempt {attempt}/{retries})"
#                 )
#                 time.sleep(random.uniform(1, 3))
#         logging.error(f"Failed to fetch {url} after {retries} attempts.")
#         return None

# fetch_qc/http_client.py
import requests
import time
import random
import logging
